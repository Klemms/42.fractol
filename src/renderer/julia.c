...
/*   Updated: 2018/09/30 15:18:07 by cababou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		j_r_2(t_window *w, long double *x, long double *y, t_fractal *fl)
{
	while ((*x) < w->width)
	{
		fl->c_r = w->mouse_p1;
		...
		while ((fl->z_r * fl->z_r + fl->z_i * fl->z_i) < 4
			&& fl->i < w->max_iteration - 1)
		{
			fl->tmp = fl->z_r;
			fl->z_r = fl->z_r * fl->z_r - fl->z_i * fl->z_i + fl->c_r;
			fl->z_i = (fl->z_i + fl->z_i ) * fl->tmp + fl->c_i;
